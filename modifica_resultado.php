...
<div class="titulo"><span id="texto_titulo_panel" >Modificaci&oacute;n de Resultados</span></div>
...
$sql="select resultado,unidades,observaciones_gerente from tbl_resultados where id='".$_REQUEST['id_resultado']."'";
$result=mysql_query($sql);
$row=mysql_fetch_object($result);


$sql2="select ref.referencia_hombre,referencia_mujer,ref.referencia_general from tbl_referencias ref
join tbl_analisis ana join tbl_categoriasanalisis cat on ana.id_analisis=cat.id and ref.id_analisis=cat.id 
where ana.id='".$_REQUEST['id_analisis']."'";
...
<tbody>
        ...
        ?>
                <td valign="center" class="Arial14Negro"><input id="txt_unidades" class="inputbox" type="text" value="<?=$row->unidades;?>" /></td>        
        <?}}?>   
</tbody>
</table>
...
<input id="btn_modificarres" type="submit" value="Guardar" name="submit" class="submit" />
<input type="hidden" id="id_resultado"  value="<?=$_REQUEST['id_resultado']?>">
</div>    
